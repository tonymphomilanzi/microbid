...
const STREAMING_KIT_PLATFORM_VALUE = "Streaming Kit";
...
function isStreamingKitCategory(cat) {
...
export default function CreateListingForm({ initial }) {
  ...
  const [lastNonKitPlatform, setLastNonKitPlatform] = useState("");
      ...
      try {
        ...
        setForm((f) => {
          ...
          return {
            ...
            metrics: f.metrics ?? defaultMetrics(defaultPlatform),
          };
        });
      } finally {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      income: initial.income == null ? "" : String(initial.income),
      expense: initial.expense == null ? "" : String(initial.expense),
      discountPercent: initial.discountPercent == null ? "" : String(initial.discountPercent), // ✅ NEW
  ...
  const isStreamingKit = useMemo(() => isStreamingKitCategory(selectedCategory), [selectedCategory]);

  // Track last non-kit platform
  useEffect(() => {
    if (!form.platform) return;
    if (isStreamingKit) return;
    if (form.platform === STREAMING_KIT_PLATFORM_VALUE) return;
    setLastNonKitPlatform(form.platform);
  }, [form.platform, isStreamingKit]);

  // Force platform for Streaming Kit + hide irrelevant fields
  useEffect(() => {
    ...
    if (isStreamingKit) {
      setForm((f) => ({
        ...f,
        platform: STREAMING_KIT_PLATFORM_VALUE,
        income: "",
        expense: "",
        metrics: null,
      }));
    ...
    // leaving streaming kit -> restore platform
    setForm((f) => {
      if (f.platform !== STREAMING_KIT_PLATFORM_VALUE) {
        if (f.id) return f;
        return { ...f, metrics: f.metrics ?? defaultMetrics(f.platform || "YouTube") };
      }

      const fallback = lastNonKitPlatform || (platforms?.[0]?.name ?? "YouTube");
      return {
        ...f,
        platform: fallback,
        metrics: f.id ? f.metrics : defaultMetrics(fallback),
      };
    });
  }, [isStreamingKit, metaLoading, lastNonKitPlatform, platforms]);

  // Reset metrics when platform changes (new listing only) — not for streaming kits
  useEffect(() => {
    if (form.id) return;
    if (!form.platform) return;
    if (isStreamingKit) return;
  ...
  async function onSubmit(e) {
    ...
    // discountPercent optional; only used for streaming kit
    const dpTrim = String(form.discountPercent ?? "").trim();
    const dpValue = dpTrim === "" ? null : Math.trunc(Number(dpTrim));

    if (isStreamingKit && dpValue !== null && (!Number.isFinite(dpValue) || dpValue < 0 || dpValue > 90)) {
      setError("Discount must be a number between 0 and 90.");
      return;
    }
    ...
    try {
      const payload = {
        ...
        income: isStreamingKit ? null : String(form.income).trim() === "" ? null : Number(form.income),
        expense: isStreamingKit ? null : String(form.expense).trim() === "" ? null : Number(form.expense),
        metrics: isStreamingKit ? null : form.metrics ?? null,

        discountPercent: isStreamingKit ? dpValue : null, // ✅ NEW
  ...
  const hasStreamingKitOption = useMemo(() => {
    return (platforms || []).some((p) => String(p?.name) === STREAMING_KIT_PLATFORM_VALUE);
  }, [platforms]);
      ...
      <CardContent>
        <form onSubmit={onSubmit} className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <select
                    className="mt-1 h-10 w-full rounded-md border border-border/60 bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring disabled:opacity-70"
                    value={form.platform}
                    onChange={(e) => setForm((f) => ({ ...f, platform: e.target.value }))}
                    disabled={metaLoading || isStreamingKit}
                  >
                    {isStreamingKit && !hasStreamingKitOption ? (
                      <option value={STREAMING_KIT_PLATFORM_VALUE}>{STREAMING_KIT_PLATFORM_VALUE}</option>
                    ) : null}
                ...
                <div className="sm:col-span-2">
                  ...
                  <Input
                    ...
                    required
                  />
                  <p className="mt-1 text-xs text-muted-foreground">Enter the full price in USD (numbers only).</p>
                </div>

                {/* ✅ NEW: Discount only for Streaming Kit */}
                {isStreamingKit ? (
                  <div className="sm:col-span-2">
                    ...
                    />
                    <p className="mt-1 text-xs text-muted-foreground">
                      Optional discount for this product. Enter a number between 0 and 90.
                    </p>
                  </div>
                ) : null}

                {/* Hide Income/Expenses for streaming kits */}
                {!isStreamingKit ? (
            ...
            {/* METRICS (hidden for Streaming Kit) */}
            {!isStreamingKit ? (
              <section className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          metrics: { ...(f.metrics ?? defaultMetrics(f.platform)), followers: Number(e.target.value) },
                        }))
                      }
                    />
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          metrics: { ...(f.metrics ?? defaultMetrics(f.platform)), avgViews: Number(e.target.value) },
                        }))
                      }
                    />
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          metrics: {
                            ...(f.metrics ?? defaultMetrics(f.platform)),
                    ...
                    <div className="flex items-center justify-between rounded-md border border-border/60 bg-muted/20 px-3 py-2">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(v) =>
                          setForm((f) => ({
                            ...f,
                            metrics: { ...(f.metrics ?? defaultMetrics(f.platform)), monetized: v },
                          }))
                        }
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          metrics: { ...(f.metrics ?? defaultMetrics(f.platform)), niche: e.target.value },
                        }))
                      }
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          metrics: { ...(f.metrics ?? defaultMetrics(f.platform)), countryTop: e.target.value },
                        }))
                      }
                    />
                  </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Use a clear screenshot of analytics or branding.</p>
            </div>

            <ImageUpload value={form.image} onChange={(url) => setForm((f) => ({ ...f, image: url }))} mode="cover" />

            <Separator className="bg-border/60" />

            <div>
              <div className="text-sm font-semibold">More images (optional)</div>
              <p className="text-xs text-muted-foreground">Add up to {MAX_EXTRA_IMAGES} extra images</p>
