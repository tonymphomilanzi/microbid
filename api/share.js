...
<!-- For humans: auto-redirect to SPA -->
<meta http-equiv="refresh" content="0; url=${escapeHtml(redirectTo)}" />
</head>
<body style="font-family:system-ui, -apple-system, Segoe UI, Roboto, Arial; padding:24px;">
  <h1 style="margin:0 0 8px 0;">${t}</h1>
  <p style="margin:0 0 16px 0; color:#666;">${d}</p>
  <p style="margin:0;">
    Redirecting… If you are not redirected, <a href="${escapeHtml(redirectTo)}">click here</a>.
  </p>
</body>
</html>`;
}
...
export default async function handler(req, res) {
  try {
    ...
    // We will use:
    // /share/feed?id=POST_ID   -> type=feed
    // /share/listing?id=LISTING_ID -> type=listing
    ...
    if (type === "feed") {
      ...
      const shareUrl = `${baseUrl}/share/feed?id=${encodeURIComponent(post.id)}`;
      const redirectTo = `/feed?post=${encodeURIComponent(post.id)}`;

      res.setHeader("Content-Type", "text/html; charset=utf-8");
      // cache short so edits reflect quickly
    ...
    // LISTING SHARE (for later; works now if you call it)
    if (type === "listing") {
      ...
      const shareUrl = `${baseUrl}/share/listing?id=${encodeURIComponent(listing.id)}`;
